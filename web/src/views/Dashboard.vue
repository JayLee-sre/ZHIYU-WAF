<template>
  <div class="v2-dashboard">
    ...
    <div class="system-strip" :class="{ degraded: system.firewall?.degraded }">
      ...
      <strong>{{ system.firewall?.available ? 'nftables 防火墙已连接' : '应用层防护运行中' }}</strong>
      <span>{{ system.firewall?.message || '本地规则、风险引擎与审计存储可用' }}</span>
    ...
    <section class="panel trend-panel">
      ...
      <div v-if="series.length" class="trend-bars">
      ...
      <div v-else class="empty">当前时间范围内尚无 V2 事件。流量经过防护链后会在这里显示趋势。</div>
